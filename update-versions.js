...
// 主函数
function main() {
  log('开始更新版本号和依赖关系...', colors.bright + colors.blue);
  ...
  for (const packageName of PACKAGES) {
    try {
      ...
      updatedVersions.push(`${packageName}@${newVersion}`);
  ...
  // 打印总结
  log('\n===== 执行结果 =====', colors.bright);
  log(`已更新以下包的版本: ${updatedVersions.join(', ')}`, colors.green);
  log('\n提示: 若要构建和发布这些包，请执行 ./update-and-publish.js', colors.yellow);
}
